...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/06/20 18:41:30 by oboutarf          #+#    #+#             */
/*   Updated: 2022/06/23 16:58:08 by oboutarf         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (!buf[fd])
		m_buf(buf, fd);
	lgn = NULL;
	lgn = ft_strjoin(lgn, buf[fd]);
	while (!ft_strbn(lgn))
	{
		vrf = read(fd, buf[fd], BUFFER_SIZE);
		if (vrf >= 0)
			buf[fd][vrf] = '\0';
		if (vrf <= 0 || buf[fd] == 0)
			return (lgn);
